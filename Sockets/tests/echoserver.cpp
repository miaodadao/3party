...
class MySocket : public Sockets::TcpSocket
{
public:
  MySocket(Sockets::ISocketHandler& h) : Sockets::TcpSocket(h) {
    SetLineProtocol();
#ifdef HAVE_OPENSSL
    if (g_b_ssl)
      EnableSSL();
...
int main(int argc,char *argv[])
{
  ...
  Sockets::StdoutLog *log = enableLog ? new Sockets::StdoutLog() : NULL;
  Sockets::SocketHandlerEp h(log);
  Sockets::ListenSocket<MySocket> l(h);
  Sockets::ListenSocket<Sockets::HttpDebugSocket> l2(h);
