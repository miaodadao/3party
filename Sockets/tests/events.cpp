...
{
	struct timeval tv;
	Sockets::Utility::GetTime(&tv);
	return tv.tv_sec + (double)tv.tv_usec / 1000000;
}


class MyEvHandler : public Sockets::EventHandler
{
public:
	MyEvHandler() : EventHandler() {}
	MyEvHandler(Sockets::StdLog *p) : EventHandler(p) {}

};


class EvThread : public Sockets::Thread
{
public:
	EvThread(Sockets::Socket *p) : m_socket(p) {}
...
class EvSocket : public Sockets::TcpSocket, public Sockets::IEventOwner
{
public:
	EvSocket(Sockets::ISocketHandler& h) : Sockets::TcpSocket(h)
	, Sockets::IEventOwner( static_cast<MyEvHandler&>(h) ) {
	}
...
void EvThread::Run()
{
	Sockets::Socket *p0 = (Sockets::Socket *)m_socket;
...
// myTimer class
//------------------------------------------------------------
class myTimer : public Sockets::IEventOwner
{
public:
	// Prototype of user call back function
	typedef void (*myTimer_cb)(myTimer* user_func, void* user_id);

	myTimer(Sockets::IEventHandler& h, long ssec, long susec,
...
//------------------------------------------------------------
myTimer::myTimer(Sockets::IEventHandler& h, long ssec, long susec,
...
{
	Sockets::Debug deb("myTimer::OnEvent()");
	if (periodic_)
	{
...
{
	Sockets::Debug deb("myTimer::myfunc()");
	deb << "Event " << ((char*)user_id) << Sockets::Debug::endl();
}
...
int main(int argc, char *argv[])
{
	...
	MyEvHandler h(&log);
	Sockets::ListenSocket<EvSocket> l(h);
